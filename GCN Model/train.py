...
def add_arguments(args):
    ...
    args.add_argument('--lr', type=float, help="learning rate", default=0.0007109804581479025) #Altered original learning rate
    args.add_argument('--min_lr', type=float, help="min lr", default=1e-4)
    args.add_argument('--bs', type=int, help="batch size", default=19) #Altered original batch size
...
#Start of code that will output predicted probabilities that an author wrote a particular paper
if __name__ == "__main__":
    ...
    #Open the training data from its appropriate directory
    ...
    #Changed the optimizer to Stochastic Gradient Descent instead of Adam
    #Altered the momentum and weight decay parameters
    optimizer = torch.optim.SGD([{'params': encoder.parameters(), 'lr': args.lr, 'momentum': 0.1257953560916134, 'weight_decay':0.05133159471116965}])
    optimizer.zero_grad()

    max_step = int(len(train_data) / args.bs * 10)
    logger.info("max_step: %d, %d, %d, %d"%(max_step, len(train_data), args.bs, args.epochs))

    #Changed to a cyclic learning rate scheduler instead of the originally defined schedule
    scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=args.lr, max_lr=0.01)
    
    #Begin the training of the GCN model on the training data
    encoder.train()
    ...
    for epoch_num in range(args.epochs):
        ...
        #Evaluate the testing loss, auc, and maps after each epoch
        if (epoch_num + 1) % args.verbose == 0:
            ...
            #Update the max_auc tracker if the new auc is greater than the previous max
            if auc > max_auc:
                early_stop_counter = 0
                ...
                #Increase early stop counter if epoch does not improve auc
                #Once the early stop counter meets or exceeds the patience, then we stop training
    ...
    if args.test_dir is not None:
        ...
        #Save our predictions into a json file
